...
A library for documenting and testing examples

This is an experiment modifying [skupperproject/skewer: A library for documenting and testing Skupper examples]( https://github.com/skupperproject/skewer ) for general purpose use without requiring Kubernetes or Podman sites.

A `skewer.yaml` file describes the steps and commands to achieve an
objective, for example, a user story.  Skewer takes the `skewer.yaml` file as input
...
[Example `skewer.yaml` file](skewer.yaml)
...
Add the Skewer code as a subdirectory:
    ...
    mkdir -p external
    curl -sfL https://github.com/pwright/skewer/archive/v2scheider.tar.gz | tar -C external -xz
...
commands:
  {command}
    ...
    demokind            Run the example steps using kind and pause for a demo before cleaning up
    demobash            Run the example steps using bash and pause for a demo before cleaning up    
...
Podman and bash sites must have a `SKUPPER_PLATFORM` variable with the value
`podman`.
